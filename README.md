...
The [EUR-LEX search page](https://do-me.github.io/roaring-static-search/) is deployed from this repository. Its index is a separate `search-index` branch of the EUR-LEX Hugging Face repository; source data and the normal dataset publishing job remain independent. The default preview returns the first 50 exact matches in stable source order and automatically loads another 50 when the result table reaches its bottom; complete-result mode exhausts the query immediately. Search results are displayed with [Hightable](https://github.com/hyparam/hightable). Its virtualized DataFrame resolves all 14 EUR-LEX columns only for viewport rows, directly from immutable source Parquet, and caches each fetched source file in the browser tab. The deployed page uses a phrase-verification batch of 96, the best observed value in the EUR-LEX feasibility benchmark; generic library users retain the conservative default of 64.

After a search, the page can complete the exhaustive exact result set, hydrate every source column, and create a private DuckDB-Wasm table named `search_results`. SQL previews, configurable bar or stacked-bar charts, and full-query Parquet, CSV, and Excel exports run in the browser. Excel cells over 32,767 characters are replaced with `exceeding Excel limits`; use CSV or Parquet for complete text. Regular charts show one bar per SQL row. Stacked charts group rows with the same X label, sum repeated X/category pairs, and colour segments by a chosen stack-by column; for example, return `year`, `document_type`, and `document_count` from SQL. Use `ORDER BY` in SQL for bar order. Charts have a 500-row safety limit; previews show at most 200 rows, while downloads run the full query. DuckDB is lazy-loaded only when the analysis workspace is requested; very broad result sets remain subject to browser memory limits.

The demo runs bitmap search, phrase verification, Parquet decoding, DuckDB setup and result conversion, chart preparation, and exports in dedicated browser workers so the page remains responsive. Complete SQL previews (up to 200 rows) are cached for instant chart creation; larger chart outputs are fetched once up to the 500-row safety limit and reused when changing axes. Loading indicators distinguish search, visible-column hydration, and analysis work.
